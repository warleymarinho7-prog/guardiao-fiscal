...
// ===== PARSER + MOTOR FISCAL =====
// [AUDIT: Motor v8 - sem alterações necessárias]
// As funções do motor fiscal (eAnalyzeSingle, eConsolidate, parsers, etc.)
// permanecem idênticas ao original — foram auditadas e aprovadas.
// Ver documento original do app.js para o código completo do motor.
// Cole aqui o bloco completo de: eParseDate até gerarRelatorioPDF
// (aproximadamente linhas 200–3800 do app.js original)
...
let _eSources=null;
// [FIX-RACE] Flag para evitar duplo clique em eRunAll
...
async function eRunAll(){
  ...
  eShowErr('');
  try {
    // ... (resto do eRunAll idêntico ao original)
    // Cole aqui o corpo completo de eRunAll do app.js original
  } finally {
...
// ===== FEEDBACK =====
window._gFeedback = { sessao: null, historico: [] };
...
// MutationObserver com disconnect após injeção do card
// [FIX-OBSERVER] observer.disconnect() chamado após injetar o card — evita leak
(function() {
  const observer = new MutationObserver(() => {
    const realBlock = document.getElementById('realResultBlock');
    if (!realBlock || realBlock.style.display === 'none') return;
    if (document.getElementById('gFeedbackCard')) return;

    const target = document.getElementById('pAlertList')?.parentElement || realBlock;
    const r = window._eConsolidated;
    if (r) {
      gRenderFeedbackCard(target, r);
      // [FIX-OBSERVER] Desconecta após injetar — sem mais observação desnecessária
      observer.disconnect();
    }
  });
...
// ===== FERRAMENTAS DE DIAGNÓSTICO (apenas desenvolvimento) =====
window.gSyntheticTest = function(perfilKey) {
  // Cole aqui o gSyntheticTest completo do app.js original
  console.log('gSyntheticTest: cole o código completo do app.js original');
};
window.gMotorDiag = function(resultado) {
  // Cole aqui o gMotorDiag completo do app.js original
  console.log('gMotorDiag: cole o código completo do app.js original');
};
window.gListProfiles = function() {
  console.log('Perfis: freelancer, mei, socio, clt_extra, investidor');
};

// ===== VERIFICAÇÃO DE PLANO ANTES DO UNLOCK =====
// [AUDIT-FIX] _verifyPlanBeforeUnlock estava ausente — adicionado
...
function eToggleDbg(){
  const b=document.getElementById('pDbgBd');
  const open=b.classList.toggle('open');
  document.getElementById('pDbgTog').textContent=open?'▲ recolher':'▼ ver';
}
