...
// ═══════════════════════════════════════════════
// LAZY LOAD — auth/checkout/extrato carregam sob demanda
// ═══════════════════════════════════════════════
let _authModuleLoaded = false;
let _extratoModuleLoaded = false;

function _loadAuthModule() {
  if (_authModuleLoaded) return Promise.resolve();
  return new Promise((resolve) => {
    const s = document.createElement('script');
    s.src = '/app-auth.js';
    s.defer = true;
    s.onload = () => { _authModuleLoaded = true; resolve(); };
    s.onerror = () => resolve(); // falha silenciosa
    document.head.appendChild(s);
  });
}

function _loadExtratoModule() {
  if (_extratoModuleLoaded) return Promise.resolve();
  return new Promise((resolve) => {
    const s = document.createElement('script');
    s.src = '/app-extrato.js';
    s.defer = true;
    s.onload = () => { _extratoModuleLoaded = true; resolve(); };
    s.onerror = () => resolve();
    document.head.appendChild(s);
  });
}

// Stubs — delegam para módulo lazy quando chamados
function openLogin() {
  _loadAuthModule().then(() => { if (typeof _openLoginImpl === 'function') _openLoginImpl(); });
}
function openCheckout(plan) {
  _loadAuthModule().then(() => { if (typeof _openCheckoutImpl === 'function') _openCheckoutImpl(plan); });
}
function doLogin() {
  _loadAuthModule().then(() => { if (typeof _doLoginImpl === 'function') _doLoginImpl(); });
}
function doCadastro() {
  _loadAuthModule().then(() => { if (typeof _doCadastroImpl === 'function') _doCadastroImpl(); });
}
function closeLogin(e) {
  if (typeof _closeLoginImpl === 'function') _closeLoginImpl(e);
}
function closeLoginDirect() {
  if (typeof _closeLoginDirectImpl === 'function') _closeLoginDirectImpl();
}
function closeCheckout(e) {
  if (typeof _closeCheckoutImpl === 'function') _closeCheckoutImpl(e);
}
function closeCheckoutDirect() {
  if (typeof _closeCheckoutDirectImpl === 'function') _closeCheckoutDirectImpl();
}
function doLogout() {
  if (typeof _doLogoutImpl === 'function') _doLogoutImpl();
}
function goToPay() {
  if (typeof _goToPayImpl === 'function') _goToPayImpl();
}
function extStep1Done() {
  if (typeof _extStep1DoneImpl === 'function') _extStep1DoneImpl();
}
function switchAuthTab(tab) {
  if (typeof _switchAuthTabImpl === 'function') _switchAuthTabImpl(tab);
}
function extSwitchTab(tab) {
  if (typeof _extSwitchTabImpl === 'function') _extSwitchTabImpl(tab);
}
